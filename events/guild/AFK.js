...
module.exports = async client => {
 client.on("message", async message => {
    ...
    let nick = db.get(`nick_${message.guild.id}_${message.author.id}`);
      ...
      return client.send(`**Chào mừng đã trở lại ${message.author}**`, message);
    }
    ...
    if (message.mentions.users.size) {
      ...
      if (mentions.size) {
        ...
        if (victim) {
          ...
          let time = db.get(`thời gian_${message.guild.id}_${victim.id}`);
          time = Date.now() - time;
          return client.send(
            `**${victim.username} Đang AFK (Tạm rời) - ${reason} - ${format(
              time
            )} Trước**`,
            message
          );
