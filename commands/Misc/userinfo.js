...
module.exports = {
  ...
  category: "info",
  description: "Nhận thông tin về một người dùng trong server",
  run: async (client, message, args) => {
    ...
    if (user.user.presence.activities.length) {
      ...
      for (let i = 0; i < data.length; i++) {
        let name = data[i].name || "Không";
        let xname = data[i].details || "Không";
        let zname = data[i].state || "Không";
...
ID: \`${user.user.id}\`
Người phân biệt đối xử: \`${user.user.discriminator}\`
Bot: ${user.user.bot}
Người dùng đã xóa: ${user.deleted}
Vị trí: ${joinPosition || 1}
      `
      );
    ...
    embed
      ...
      .addField("Quyền hạn", `**\`${permissions.join("\n")}\`**`)
