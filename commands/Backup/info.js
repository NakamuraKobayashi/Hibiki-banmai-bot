...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    let backupID = args[0];
    if (!backupID) {
      return client.send(":x: | Bạn phải chỉ định ID Backup hợp lệ!", message);
    }
    // Fetch the backup
    backup
      .fetch(backupID)
      .then(backupInfos => {
        ...
        let embed = new Discord.MessageEmbed()
          ...
          .addField("Tạo lúc", formatedDate, false)
          .setColor("#FF0000");
      ...
      .catch(err => {
        ...
        return client.send(
          ":x: | Không tìm thấy backup nào cho ID `" + backupID + "`!",
          message
        );
      });
  }
